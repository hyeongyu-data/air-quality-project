...
producer/timeutil.py와 같은 내용이다. Consumer 이미지는 consumer/ 디렉터리만
포함하므로(Dockerfile) producer 쪽을 import 할 수 없다. 공용 패키지를 두면
두 런타임의 sys.path를 모두 손봐야 해서, 이 정도 크기는 복제가 더 싸다.
