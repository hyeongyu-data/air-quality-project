...
> 현재 작업 상태(2026-09-10): P0-2에서 카카오 토큰·쿨다운 캐시를 `/app/state` 명명 볼륨으로 영속화했다. 이 문서의 과거 이슈 서술은 당시 트리거와 변경 이력을 보존하므로 현재 코드와 다를 수 있으며, 최신 상태는 이 주석과 `docs/DEVELOPMENT_PLAN.md`를 우선한다.
